Macropad:


Demo:

https://streamable.com/4rilwd


3D look of the assembled macropad:

<img width="1137" height="732" alt="Screenshot 2026-07-23 165033" src="https://github.com/user-attachments/assets/a6909550-dc2f-4371-9b34-302b15457286" />


3D look of the macropad wihtout the top attached. PCB view:

<img width="907" height="612" alt="Screenshot 2026-07-23 165354" src="https://github.com/user-attachments/assets/a2702550-5b93-4258-bdfb-ad7fc8ffcab6" />


Schematic:

<img width="1007" height="822" alt="image" src="https://github.com/user-attachments/assets/2e13fcb2-60fc-467c-b171-a967e10afdf1" />


PCB:

<img width="742" height="780" alt="image" src="https://github.com/user-attachments/assets/dd5077d2-1577-4349-8335-c11e74045354" />


PCB 3D view:

<img width="665" height="747" alt="image" src="https://github.com/user-attachments/assets/34b283d7-e27d-46aa-ad21-35b8491a6f91" />


Case:

<img width="776" height="432" alt="Screenshot 2026-07-20 025003" src="https://github.com/user-attachments/assets/aedda8ca-82f8-419e-b9fb-c3319d62225d" />


Firmware:

The micropad uses QMK Firmware.


Description:

A custom handwired 10-key macropad running QMK firmware on a Seeed Studio XIAO RP2040 development board. 

* Keyboard Maintainer: [mishelkora11](https://github.com/mishelkora11)
* Hardware Supported: Seeed Studio XIAO RP2040, MX-style switches, and an 0.91 inch OLED Display.


BOM:

1x seeed studio rp2040

4x  style switches

1x 0.91 inch oled display

4x M3x5x4 Heatset inserts

1x Case 

4x M3x16mm screws



